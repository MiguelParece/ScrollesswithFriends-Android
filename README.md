...
- **Minimal mode** (Settings → *Minimal mode*) — the inverse of blocking a list: during hours you choose, *everything* closes itself except an allowlist you pick. Your allowlist starts empty, so nothing is assumed on your behalf — not even WhatsApp. Calls, the emergency dialer, alarms, the home screen, the keyboard, the system UI and Scrolless itself are always allowed and cannot be removed; Settings deliberately is not, since that is where blocking gets switched off. The schedule reads a clock derived from elapsed-realtime, so winding the device clock does not move your hours. While strict mode is armed the switch can only go on, apps can only be removed from the allowlist, and the hours can only grow.
