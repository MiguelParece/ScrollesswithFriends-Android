...
The original Scrolless blocks Reels/Shorts/TikTok on your own willpower's terms. This fork adds two things on top: a **social layer**, where your scroll budget is small and only *other people* can top it up, and a set of **commitment devices** you can point at yourself — a time lock you cannot undo, and hours where the phone stops being a phone full of apps.
...
- **Block All is now an allowlist** — it used to mean "block every supported short-form surface". It now means what it says: while it is the chosen mode, *every* app that is not on your allowlist closes itself. Configure it in Settings → *Minimal mode*. Your allowlist starts empty, so nothing is assumed on your behalf — not even WhatsApp. Calls, the emergency dialer, alarms, the home screen, the keyboard, the system UI and Scrolless itself are always allowed and cannot be removed; Settings deliberately is not, since that is where blocking gets switched off. Short-form content stays blocked inside the apps you *do* allow, so allowing Instagram gives you DMs, not the feed.
- **Optional hours** — by default Block All applies all day. Add hours to narrow it to part of the day instead. The schedule reads a clock derived from elapsed-realtime, so winding the device clock does not move it. While strict mode is armed, apps can only be removed from the allowlist and the hours can only grow — and since Block All is the strictest mode, strict mode will not let you leave it either.
- **What the allowlist does not stop** — it closes an app about a quarter-second *after* it opens, so you see it briefly; Recents and the notification shade still show content; widgets and the lock screen are not apps and are not guarded; and any app you add to the allowlist is fully open, WhatsApp included, Status and Channels and all. It is a deterrent with real friction, not a kiosk — a Device Owner setup over ADB is the only way to get the system itself to enforce this.
...
- **Still zero permissions, still fully offline** — the app declares no permissions beyond the accessibility service and even strips the INTERNET permission at build time via the manifest merger. Gift links travel through *your* messaging apps; Scrolless itself never touches the network. The one capability minimal mode added is a `<queries>` manifest entry so the app picker can list apps that have a launcher icon — not `QUERY_ALL_PACKAGES`, which would hand it an inventory of everything installed.

Roadmap ideas: clock-tamper hardening for the original Daily Limit / Interval modes, a Device Owner variant of minimal mode for people willing to do the ADB setup (the system would then refuse to open blocked apps at all, instead of Scrolless closing them a moment later), upstream bug fixes, and whatever the next iteration session comes up with.
...
- **Block All**
  Upstream: instantly block access to all supported platforms, including Instagram Reels, TikTok, YouTube Shorts, and Facebook Reels. **In this fork it means more than that** — see the allowlist above.
