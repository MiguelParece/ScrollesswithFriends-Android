...
data class UserSettingsEntity(
    ...
    @ColumnInfo(name = "partner_quota_window_key", defaultValue = "") val partnerQuotaWindowKey: String = "",
    @ColumnInfo(name = "partner_quota_used_millis", defaultValue = "0") val partnerQuotaUsedMillis: Long = 0L,
    @ColumnInfo(name = "partner_quota_granted_millis", defaultValue = "0") val partnerQuotaGrantedMillis: Long = 0L,
    ...
    // Retired: the allowlist is armed by picking BlockAll, not by a switch of its own. The
    // column stays because dropping one in SQLite means rebuilding the table, and Room
    // rejects a database with unmapped columns.
